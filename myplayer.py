...
def is_winning_board(board, column, maximizing) -> bool:
    ...
    # check horizontal
    for start in range(column - 3, column + 1):
    ...
    for startRow in range(rowNum - 3, rowNum + 1):
        if row_in_range(startRow) and row_in_range(startRow + 3):
            for startCol in range(column - 3, column + 1):
    ...
    # check up diagonal
    for startRow in range(rowNum + 3, rowNum - 1, -1):
        if row_in_range(startRow) and row_in_range(startRow - 3):
            for startCol in range(column - 3, column + 1):
                if col_in_range(startCol) and col_in_range(startCol + 3):
                    if (board[startRow, startCol] == value_to_check
...
def alpha_beta(board, depth, alpha, beta, maximizing, lastColumn):
    ...
    if won:
        print("winning")
        if maximizing:
            return [columnToPlay, -100000000000000]
        else:
            return [columnToPlay, +100000000000000]
    ...
    columnToPlay = random.choice(validMoves)
...
class MyPlayer(Player):
    ...
    def play(self, board: np.ndarray) -> int:
        ...
        """
        choice, heuristic =  alpha_beta(board, 5, 1000, -1000, True, None)
        print('choice', choice)
